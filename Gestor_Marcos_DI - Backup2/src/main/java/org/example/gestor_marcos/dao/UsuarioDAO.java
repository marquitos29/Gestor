...
public class UsuarioDAO {
    ...
    public int comprobarUsuario(String correo, String pass, int rango) throws SQLException {
        if (rango == 1) {
            ...
            if (resultSet.next()) return resultSet.getInt(DBSchema.COL_ID_THE);
            else return -1;
            ...
            preparedStatement.setString(2, pass);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()) return resultSet.getInt(DBSchema.COL_ID_STU);
            else return -1;
        }
    }
    ...
    public int getIdUsuario(String correo) throws SQLException {
        try {
            ...
            String queryProfesor = String.format(
                    "SELECT %s FROM %s WHERE %s = ?",
                    DBSchema.COL_ID_THE, DBSchema.TAB_THE, DBSchema.COL_MAIL_THE, DBSchema.COL_PASS_THE
            );
            PreparedStatement preparedStatement = connection.prepareStatement(queryProfesor);
            preparedStatement.setString(1, correo);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()) return resultSet.getInt(DBSchema.COL_ID_THE);
            throw new SQLException("No se encontró el id de este usuario.");
        }
        return 0;
    }
    ...
    public List<Map<String, String>> obtenerAlumnosPorProfesor(int idProfesor) throws SQLException {
        ...
        String query = String.format(
                "SELECT a.%s, a.%s, a.%s, n.%s AS nota " +
                        "FROM %s s " +
                        "INNER JOIN %s c ON s.%s = c.%s " +
                        "INNER JOIN %s a ON c.%s = a.%s " +
                        "LEFT JOIN %s n ON n.%s = CONCAT(s.%s, '_', a.%s) " +
                        "WHERE s.%s = ?",
                DBSchema.COL_NAME_STU, DBSchema.COL_SNAME_STU, DBSchema.COL_MAIL_STU, "nota",
                ...
                "notas", "clave", DBSchema.COL_NAME_SUBJECT, DBSchema.COL_NAME_STU, DBSchema.COL_FK_TEACHER_USER_SUBJECT
        );
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    alumno.put("nota", resultSet.getString("nota") != null ? resultSet.getString("nota") : "0");
