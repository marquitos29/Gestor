...
public class LoginController implements Initializable, EventHandler<ActionEvent> {
    ...
    public void manejarAcceso() {
        ...
        if (correo.isEmpty() || pass.isEmpty()) {
            ...
            try {
                int perfil = usuarioDAO.getIdFk(usuarioDAO.getIdUsuario(correo));
                id = usuarioDAO.comprobarUsuario(correo, pass, perfil);
                generarNotasAleatorias();
                if (perfil == 1 && id != -1) {
                    System.out.println("Id del perfil: " + usuarioDAO.getIdUsuario(correo));
                    System.out.println("Tipo de perfil: " + usuarioDAO.getIdFk(usuarioDAO.getIdUsuario(correo)));
                    cargarVistaProfesor();
                ...
                mostrarAlerta("Error al verificar el usuario en la base de datos.");
            }
        }
    }
